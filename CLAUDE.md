...
**Achieved accuracy:** 100% detection, 1.25% FA (real testbed hold-out, exceeding Hachimi et al. 2020's 94.51%)
**Stage 3 accuracy:** 85% overall (PSS 94%, PDCCH 72%, DMRS 84%, Deceptive 90%) — 실측 I/Q 200개 평가
**Latency:** ≤ 23 ms (p95) end-to-end, well within Near-RT RIC 100 ms budget
...
[Stage 1: MLP — 5-class]
    ...
    └─ Normal                        → [Stage 2: KSVM sliding window (W=15, 12-dim 통계)]
                                           ├─ Normal → CLEAN
...
Stage 3 MobileNetV3 uses **2D spectrograms** (224×224) from raw I/Q:
- **학습:** 합성 1,200개 pretrain → 실측 I/Q 200개 fine-tune (Instance-3 full-duplex 캡처)
- **평가:** 실측 I/Q 200개 (PSS 94%, PDCCH 72%, DMRS 84%, Deceptive 90%, 전체 85%)
- **Production:** on-demand I/Q snapshot (Stage 1이 Deceptive 또는 Stage 2가 FN 포착 시에만 호출)
...
└── tools/
    ...
    ├── iq_capture_fullduplex.py ← [Phase 7] Instance-3 full-duplex TX+RX 동시 캡처
    ├── collect_iq_all.sh      ← [Phase 5] 전모드 I/Q 자동 수집 (Instance-1)
    ├── collect_iq_instance3.sh ← [Phase 7] Instance-3 full-duplex 8모드 자동 수집
...
| **UHD** | 4.6.0 | 전 Instance 동일 (srsUE/gNB 모두 libuhd.so.4.6.0 링크) |

### 8.1 Jammer 고정 설정 (Phase 4 실측 확정)
...
#### 모드별 Gain/Amplitude (Phase 5 실측 확정)

| 재밍 모드 | Gain | Amplitude | 비고 |
|---|---|---|---|
| **Constant** | 0 | **0.5** (안정) / 0.6 (최대) | 0.5: UE 유지+BLER 75%. 0.6: UE ~30초 후 탈락 위험 |
| **Random** | 0 | 0.6 | `build_random_frame(duty=30%)` 사용. BLER mean=0.33 |
| **Reactive** | 0 | 0.6 | duty ~40%, BLER mean=0.28 |
| **Deceptive** | 0 | 0.6 | OFDM 위장, KPM 영향 미미 |
| **PSS** | 10 | 1.0 | 협대역 SSB 타겟, 고출력 필요 |
| **PDCCH** | 5 | 1.0 | CORESET 타겟 |
| **DMRS** | 10 | 1.0 | 빗살 패턴 파일럿 타겟, 고출력 필요 |
